...
11. Run the full suite, lint, and format with spgr-run-tests and confirm clean and the validation test green before write. Write each factory file, the validation test, and the reference table through spgr-write-file. Record any non-obvious factory design choice with spgr-log-decision. When a factory touches a vertical's domain (a PII field or an access-control state), consult that specialist with spgr-tag-vertical-agent before treating it as done.
