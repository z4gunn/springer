...
3. Author the staged pipeline with fail-fast ordering:
   ...
   - Stage 2 Lint: run the linter and formatter check, fail on any lint error or unformatted file. Run this in parallel with unit tests, the one allowed exception to fail-fast since both are fast and independent.
