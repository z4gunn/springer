...
10. Run the consumer suite, the provider suite, lint, and format with spgr-run-tests and confirm clean before write. For TypeScript or JavaScript, conform to `.claude/references/typescript-standards.md` and pass `tsc --noEmit`. Write each test file and the pact file through spgr-write-file. Record any non-obvious contract design choice with spgr-log-decision. Commit one logical change per commit.
