...
2. Identify every layer the architecture requires. For each layer, produce one entry. Do not select two technologies that solve the same problem within a layer. If the architecture implies that overlap, resolve it to one choice and record the reason in the entry.
...
- The tech-stack-decision artifact type is registered in the schema registry at `/Users/gunderer/Repos/springer/schemas/`. Reference field requirements through spgr-validate-artifact rather than inlining them here.
