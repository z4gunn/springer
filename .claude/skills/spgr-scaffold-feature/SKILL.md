...
7. Run the project formatter and linter on the generated stubs and confirm a clean pass before handoff. For TypeScript or JavaScript, conform to `.claude/references/typescript-standards.md` and pass `tsc --noEmit`. Fix any stub that fails. Do not hand off a scaffold that is not lint and format clean.
