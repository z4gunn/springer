...
7. Run the full suite again with spgr-run-tests, then run the linter and formatter. For TypeScript or JavaScript, conform to `/Users/gunderer/Repos/springer/.claude/references/typescript-standards.md` and pass `tsc --noEmit`. The story is not ready until all acceptance tests pass, all unit tests pass, the linter passes, the formatter passes, and no new lint errors were introduced.
...
- The acceptance tests, the implementation, and the unit tests are source code verified by spgr-run-tests and CI, not by an envelope schema. Only the pull request is an envelope artifact, validated against its registered schema in /Users/gunderer/Repos/springer/schemas/ through spgr-validate-artifact.
