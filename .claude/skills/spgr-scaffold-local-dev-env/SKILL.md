...
3. Write the `Makefile` via spgr-write-file with idempotent targets. `make install` provisions tooling and dependencies and re-runs cleanly after a dependency change rather than failing. For a JavaScript-runtime stack, the linter, formatter, and TypeScript toolchain it provisions are the gts baseline defined in `/Users/gunderer/Repos/springer/.claude/references/typescript-standards.md`. `make dev` starts the compose stack and the application. `make test` runs the suite. `make reset` tears down and recreates a clean state. `make verify` is the acceptance gate below.
