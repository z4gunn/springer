...
- A merge is a human-in-the-loop gate. This skill opens the PR and records the artifact. It does not merge. The merge decision runs through spgr-notify-human at the merge checkpoint. The full bar a PR must clear to merge is defined in `.claude/references/git-workflow.md`.
