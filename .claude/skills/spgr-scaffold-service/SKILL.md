...
9. Validation gate. Run the smoke test with spgr-run-tests and confirm it starts the service and gets a valid health-check response. Run the project linter and formatter and confirm a clean pass on the new files. For TypeScript or JavaScript, conform to `.claude/references/typescript-standards.md` and pass `tsc --noEmit`. If any check fails, fix the service and re-run. Do not hand off a service that does not pass the smoke test on a clean run.
