...
8. Lint, format, and verify. Run the linter and formatter to clean before commit. Run spgr-run-tests until the suite passes. Build only what the acceptance criteria require. Log the consequential choices (key strategy, backoff schedule, max attempts) via spgr-log-decision.
