...
2. On an API spec change, regenerate the API reference with spgr-generate-api-docs. Scaffold or refresh SDK clients with spgr-generate-sdk using the generation tooling confirmed at architecture, for the languages confirmed at architecture.
...
4. At release time, produce the changelog with spgr-generate-changelog from the conventional commit log, each entry referencing its commit or PR. Produce the user-facing release notes as a separate artifact with spgr-generate-release-notes, with no commit hashes and no unexplained internal names.
5. Maintain the README with spgr-generate-readme, adding a deployment section and an environment variable reference with description and example per variable. Produce the contributing guide with spgr-write-contributing-guide and the audience-specific onboarding guide with spgr-write-onboarding-guide against the `onboarding_target`.
6. When a documentation-quality degradation signal arrives in `dx_friction_signals`, run the DX friction audit with spgr-audit-dx-friction, pairing each friction point with the documentation change that removes it.
