...
3. Test-first. Write the failing test before implementation. Build components with spgr-write-component implementing all states (default, loading, error, empty) and implement state with spgr-implement-state-management on the approved pattern. Use spgr-implement-feature to orchestrate the story.
4. Wire navigation and smoke-test it. Configure deep linking with spgr-configure-deep-linking and smoke-test it before the first release. Configure push notifications with spgr-configure-push-notifications. Write platform permissions with spgr-write-platform-permissions, each with rationale copy that a human reviews before submission.
...
7. Run spgr-format-code and spgr-lint-code. Consult verticals with spgr-tag-vertical-agent: App Store Compliance for HIG or Material on every UI PR, Accessibility for the a11y audit, Analytics for event instrumentation, Feature Flag for in-progress features, i18n when localization is in scope.
